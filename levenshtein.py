...
def computeScores_LS(inputDir, outCSV, acceptTypes):
    ...
    with open(outCSV, "w") as outF:
        ...
        for file1, file2 in files_tuple:
            try:
                ...
                row_ls_distance.append(levenshtein_similarity(''.join(str(file1)), ''.join(str(file2))))            

                a.writerow(row_ls_distance)  
            except ConnectionError:
                sleep(1)
            except KeyError:
                continue
...
'''
def computeScores2_LS(inputFile, outCSV):
    with open(outCSV, "w") as outF:
        a = csv.writer(outF, delimiter=',')
        a.writerow(["x-coordinate", "y-coordinate", "Similarity_score"])
        ...
        for row1, row2 in rows_tuple:
            ...
            try:
                row_ls_distance = [row_list.index(row1), row_list.index(row2)]

                #v1 = Vector(inputFile, row1, config_bik)
                #v2 = Vector(inputFile, row2, config_bik)

                row_ls_distance.append(levenshtein_similarity(''.join(str(row1)), ''.join(str(row2))))
...
if __name__ == "__main__":
    ...
    argParser = argparse.ArgumentParser('Levenshtein similarity based on Metadata values')
    ...
    argParser.add_argument('--outCSV', required=True, help='path to directory for storing the output CSV File, containing pair-wise Levenshtein similarity Scores')
    argParser.add_argument('--accept', nargs='+', type=str, help='Optional: compute similarity only on specified IANA MIME Type(s)')
    args = argParser.parse_args()

    if args.inputDir and args.outCSV:
        computeScores_LS(args.inputDir, args.outCSV, args.accept)
    if args.inputFile and args.outCSV:
        computeScores2_LS(args.inputFile, args.outCSV)